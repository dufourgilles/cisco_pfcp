...
tvb_ntp_fmt_ts_sec(tvbuff_t *tvb, gint offset)
{
        ...
        */
        ...
        buff = (char *)wmem_alloc(wmem_epan_scope(), NTP_TS_SIZE);
...
pfcp_get_frame(address ip, guint64 seid, guint32 *frame) {
    ...
    ip_str = address_to_str(wmem_epan_scope(), &ip);
...
dissect_pfcp_f_teid(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, proto_item *item, guint16 length, guint8 message_type _U_, pfcp_session_args_t *args _U_)
{
    ...
    if ((fteid_flags_val & 0x4) == 4) {
        ...
        proto_item_append_text(item, "TEID: 0x%s", tvb_bytes_to_str(wmem_epan_scope(), tvb, offset, 4));
        offset += 4;
...
decode_pfcp_network_instance(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, proto_item *item _U_, gint offset, int length)
{
    ...
    if (length > 0) {
        ...
        if (name_len < 0x41) {
            ...
            if (name_len < 0x20) {
                apn = tvb_get_string_enc(wmem_epan_scope(), tvb, offset + 1, length - 1, ENC_ASCII);
                for (;;) {
                ...
                apn = tvb_get_string_enc(wmem_epan_scope(), tvb, offset, length, ENC_ASCII);
            }
            ...
            proto_tree_add_item_ret_string(tree, hf_pfcp_network_instance, tvb, offset, length, ENC_ASCII | ENC_NA, wmem_epan_scope(), &string_value);
...
dissect_pfcp_application_id(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, proto_item *item _U_, guint16 length, guint8 message_type _U_, pfcp_session_args_t *args _U_)
{
    ...
    */
    ...
    {
        const guint8* string_value;
        proto_tree_add_item_ret_string(tree, hf_pfcp_application_id_str, tvb, offset, length, ENC_ASCII | ENC_NA, wmem_epan_scope(), &string_value);
        proto_item_append_text(item, "%s", string_value);
    }
    else
    {
...
dissect_pfcp_f_seid(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, proto_item *item, guint16 length, guint8 message_type _U_, pfcp_session_args_t *args)
{
    ...
    proto_item_append_text(item, "SEID: 0x%s", tvb_bytes_to_str(wmem_epan_scope(), tvb, offset, 8));
    offset += 8;
    /* IPv4 address (if present)*/
    if ((f_seid_flags & 0x2) == 2) {
        ipv4 = wmem_new0(wmem_epan_scope(), address);
    ...
    if ((f_seid_flags & 0x1) == 1) {
        ipv6 = wmem_new0(wmem_epan_scope(), address);
    ...
    switch (node_id_type) {
        ...
        case 2:
            ...
            if (length > 1) {
                ...
                if (name_len < 0x40) {
                    fqdn = tvb_get_string_enc(wmem_epan_scope(), tvb, offset + 1, length - 2, ENC_ASCII);
                    for (;;) {
                ...
                /* In case the FQDN field is incorrectly in dotted string form.*/
                    ...
                    fqdn = tvb_get_string_enc(wmem_epan_scope(), tvb, offset, length - 1, ENC_ASCII);
...
decode_pfcp_urr_id(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, proto_item *item _U_, guint offset)
{
    ...
    */
    urr_id_flag = tvb_get_uint8(tvb, offset) & 0x80;
...
dissect_pfcp_application_instance_id(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, proto_item *item _U_, guint16 length, guint8 message_type _U_, pfcp_session_args_t *args _U_)
{
    ...
    {
        const guint8* string_value;
        proto_tree_add_item_ret_string(tree, hf_pfcp_application_instance_id_str, tvb, offset, length, ENC_ASCII | ENC_NA, wmem_epan_scope(), &string_value);
        proto_item_append_text(item, "%s", string_value);
    }
    else
    {
...
decode_pfcp_far_id(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, proto_item *item _U_, gint offset, guint16 length)
{
     ...
     */
    far_id_flag = tvb_get_uint8(tvb, offset) & 0x80;

    proto_tree_add_item(tree, hf_pfcp_far_id_flg, tvb, offset, 1, ENC_NA);

    if (length - offset >= 4) {
        far_id = tvb_get_uint32(tvb, offset, ENC_BIG_ENDIAN) & 0x7fffffff;
        proto_tree_add_item(tree, hf_pfcp_far_id    , tvb, offset, 4, ENC_BIG_ENDIAN);
        offset += 4;
    } else if (length - offset >= 2) {
        far_id = tvb_get_uint16(tvb, offset, ENC_BIG_ENDIAN) & 0x7fff;
...
decode_pfcp_qer_id(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, proto_item *item _U_, guint offset)
{
    ...
    * by the CP Function. If set to 1, it indicates that the Rule is predefined in the UP Function
    */
    qer_id_flag = tvb_get_uint8(tvb, offset) & 0x80;
...
dissect_pfcp_cisco_source_ip_address(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree, proto_item *item _U_, guint16 length, guint8 message_type _U_, pfcp_session_args_t *args _U_)
{
    ...
    if ((flags_val & 0x4) == 4) {
        ...
        proto_item_append_text(item, "MPL %u", tvb_get_uint8(tvb, offset));
        offset += 1;
    }
...
dissect_pfcp(tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree, void *data _U_)
{
    ...
    pfcp_hdr = wmem_new0(wmem_epan_scope(), pfcp_hdr_t);
    ...
    if (g_pfcp_session) {
        args = wmem_new0(wmem_epan_scope(), pfcp_session_args_t);
        args->last_cause = 1;                                         /* It stores the last cause decoded. Cause accepted by default */
        /* We create the auxiliary lists */
        args->seid_list = wmem_list_new(wmem_epan_scope());
        args->ip_list = wmem_list_new(wmem_epan_scope());
    }
...
/*
   ...
   */
    ...
    if (PFCP_GET_COMPRESSION_ENABLED(pfcp_cisco_flags)) {    
        ...
        next_tvb = tvb_uncompress_zlib(tvb, offset,  datalen); // sx_uncompress_zlib
        if (next_tvb) {
